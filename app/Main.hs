...
import Facebook
       (Credentials(..), AccessToken(..), UserAccessToken, UserKind)

import Facebook.Haxl
...
main = do
  ...
  facebookState <- initGlobalState 10 creds manager
  env <- initEnv (stateSet facebookState stateEmpty) ()
  r <- runHaxl env $
    getPage "mabiuscenter" [] access_token
  print r
...
getCredentials :: IO (Credentials, (AccessToken UserKind))
